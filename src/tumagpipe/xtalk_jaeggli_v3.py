...
def fit_mueller_matrix(data,pthresh=0.02,norm=False,
    ...
    if method == 'jaeggli' or method == 'all_wvls' or method == 'corr':
        ...
        full_data=data.copy()
        full_data=full_data[region[0]:region[1],region[2]:region[3],:,:]

        #Last wavelength to be considered in the minimization
        full_data=full_data[:,:,:last_wvl,:]
        ...
        V_mean_corr=full_data[:,:,:,3]-np.mean(full_data[:,:,0,3],axis=(0,1))
        pmap = np.max(np.abs(V_mean_corr)/full_data[:,:,:,0], axis=2)
        notpolar = np.argwhere(pmap < pthresh)
        nyidx = notpolar[:,0]
        nzidx = notpolar[:,1]

        # Use just the region with weak polarization
        weak_region = full_data[nyidx,nzidx,:,:] #do selection for only strong polarization signals
