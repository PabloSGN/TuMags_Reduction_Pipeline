...
### Identifying individual observing modes images. 

TuMag's observing blocks are composed of several observing modes, either in a combination of two observing modes or a single observing mode repeated throughout the whole observation block. In order to be able to identify the images corresponding to a single observation mode, the images of each observation _blocks_ have to be divided into the different observation _modes_. 

There are two approaches provided by the pipeline for this purpose: 

#### Separating the obs block in terminal: 

From a terminal in the IAA's server, run the [Check_ocs.py](../Check_ocs.py) routine with the images IDs and the  *save* keyword to generate a pickle file with a dictionary with the paths separated based on their [Observation_Counter](../README.md#observation-counters): 

**EXAMPLE**
```shell
python ../TuMags_Reduction_Pipeline/Check_ocs.py D10-23675-36096 save
```

This command will start to print the progress in terminal. When finished, a summary of the observing modes should be printed:

```shell
OC : 42 - Obs Mode : 1 - Nims : 80 - Complete
OC : 43 - Obs Mode : 2.02 - Nims : 64 - Complete
OC : 44 - Obs Mode : 1 - Nims : 80 - Complete
OC : 45 - Obs Mode : 2.02 - Nims : 64 - Complete
```

The summary displays the observation counters, the observing mode and the number of images, along with a Complete/Incomplete keyword to indicate if all images of the observing mode have been found. 

Additionally, a pickle file of name "OCS.pickle" will be generated **when using the save keyword in the execution command**. 
...
After the pickle file has been generated by any of the two methods, load it into your code with:

```python
with open("ocs_pickle_file.pickle", "rb") as file:
    OCS = pickle.load(file)
```

Information about each observing mode can be accesed through the Observation Counter. 

 - OCS[OC]["line] -> Spectral line
 - OCS[OC]["OM"] -> Observing mode
 - OCS[OC]["ims] -> paths to the observing modes images. 
...
```python
OC = 170 # For example
om = ih.nominal_observation(OCS[OC]["OM"], OCS[OC]["ims"], dc = dc) 
om_data = om.get_data()
om_info = om.get_info()
```
