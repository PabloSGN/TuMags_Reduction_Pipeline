...
#        print(f"Wavelength sampling (pm): {wvlv * 1e12}")
    ...
    if norm_method == "blueshift":
        ...
        for cam in range(2):
            ...
            cam_interp = interp1d(wvlv_interp, cam_ave_interp, kind='quadratic',
            ...
            if flat_optimization:
                ...
                # Rejilla de shifts
                delta_grid = np.linspace(-10e-12, 10e-12, 51)
            ...
            # elif method_fit == 'optimized':

            #     def merit_indiv1(params, cam_indiv, wvlv_fit, interp):
            #         delta_wvl, scale = params
            #         diff = scale * interp(wvlv_fit + delta_wvl) - cam_indiv
            ...
            #                 )
            #                 guess_ij = (
            #                     [blueshift_guess[i, j], scale_guess[i, j]]
            #                     if import_blueshift_guess else [0, 1]
            #                 )
            #                 minim = minimize(
            #                     merit_indiv1,
            #                     x0=guess_ij,
            #                     args=(cam_indiv, wvlv_fit, interp),
            #                     method=meth,
            #                     bounds=[(-10e-12, 10e-12), (0.1, 2)]
            #                 )
            #                 shifts_block[ii, j] = minim.x[0]
            #                 scales_block[ii, j] = minim.x[1]
            #         return i0, i1, shifts_block, scales_block
    ...
    # # Correct zero blueshift to coincide with minimum blueshift across FOV
            ...
            # # Interpolate spectral profiles to correct for blueshift
            # for i in tqdm(range(Nx)):
            #     for j in range(Ny):
            #         cam_shifted_average = 0.0
            ...
            #         # Apply Eq. 3 from De la Cruz et al. 2015 to each modulation
            #         for k in range(N_mods):
            #             norm_ff[cam, :, k, x0+i, y0+j] = (
            #                 scales[cam, i, j] * data[cam, :, k, x0+i, y0+j] / cam_shifted_average
            #             )
